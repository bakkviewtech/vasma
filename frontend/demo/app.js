...
const DEMO_RESET_DAY_KEY = "vasma-demo-reset-day";
const DEMO_SAMPLE_VERSION_KEY = "vasma-demo-sample-version";
const DEMO_SAMPLE_VERSION = "2026-06-01-20-transactions-v2";
const STORE_KEY = "vasma-demo-service-manager-v1";
const LICENSE_STORE_KEY = "vasma-demo-license-store-v1";
const DEVICE_ID_KEY = "vasma-demo-device-id";
const SYNC_SEQUENCE_KEY = "vasma-demo-sync-server-sequence";
const LAST_SYNC_STATE_KEY = "vasma-demo-last-synced-state";
const INSTALL_DATE_KEY = "vasma-demo-install-date";
const APP_VERSION = "v102-demo";
const DEVELOPER_EDITS_KEY = "vasma-demo-developer-edits";
const DEVELOPER_MODE_KEY = "vasma-demo-developer-mode";
...
function buildDemoSampleState() {
  const demo = structuredClone(seed);
  const allowedServices = enabledServiceKeys();
  const categoryNamesByService = { tire: "Tire Service", carWash: "Car Wash", general: "General Service" };
  const allowedCategoryNames = allowedServices.map((service) => categoryNamesByService[service]).filter(Boolean);
  ...
  const serviceByName = (categoryName, itemIndex = 0) => {
    const category = demo.categories.find((entry) => entry.name === categoryName) || demo.categories[0];
    const item = category.items[itemIndex % category.items.length] || category.items[0];
    return { category, item };
  };
  demo.settings = {
    ...demo.settings,
    businessId: "DEMO-PANEL-001",
    businessCode: "DEMO-PANEL",
    businessName: "VASMA Demo Panel",
    location: "Dar es Salaam Demo Branch",
  ...
  demo.customers = [
    ["demo-cust-01", "Sample Transport Ltd", "T 123 ABC", "Mitsubishi Pickup", "Asha Mhando", "+255 712 000 000"],
    ["demo-cust-02", "Kilimanjaro Logistics", "T 448 DFG", "Toyota Hiace", "Baraka Simon", "+255 713 100 200"],
    ["demo-cust-03", "Mwenge Hardware", "T 902 KLM", "Isuzu Truck", "Neema Paulo", "+255 714 200 300"],
    ["demo-cust-04", "City Taxi Group", "T 771 QWE", "Toyota Axio", "Hassan Ally", "+255 715 300 400"],
    ["demo-cust-05", "Lake Oil Services", "T 660 ZXC", "Nissan Patrol", "Grace Joseph", "+255 716 400 500"],
    ["demo-cust-06", "Rapid Delivery", "T 818 RTY", "Suzuki Carry", "John Peter", "+255 717 500 600"]
  ].map(([id, name, vehicle, vehicleModel, contact, mobile]) => ({ id, name, vehicle, vehicleModel, contact, mobile, demoSample: true }));
  demo.employees = [
    { id: "demo-emp-01", name: "Juma Said", mobile: "+255 713 111 111", role: "Supervisor", demoSample: true },
    { id: "demo-emp-02", name: "Neema John", mobile: "+255 714 222 222", role: "Supervisor", demoSample: true },
    { id: "demo-emp-03", name: "Peter Joseph", mobile: "+255 715 333 333", role: "Supervisor", demoSample: true },
    { id: "demo-emp-04", name: "Amina Salum", mobile: "+255 716 444 444", role: "Attendant", demoSample: true }
  ];

  const categories = allowedCategoryNames.length ? allowedCategoryNames : ["Tire Service"];
  const jobAmounts = [18000, 25000, 42000, 35000, 56000, 28000, 66000, 15000, 48000, 32000, 75000, 22000, 39000, 52000, 61000, 27000, 44000, 58000, 31000, 69000];
  demo.jobCards = jobAmounts.map((amount, index) => {
    const service = serviceByName(categories[index % categories.length], index);
    const employee = demo.employees[index % demo.employees.length];
    const customer = demo.customers[index % demo.customers.length];
    const jobId = `demo-job-${String(index + 1).padStart(2, "0")}`;
    const itemId = `demo-job-item-${String(index + 1).padStart(2, "0")}`;
    const isGeneral = service.category.name === "General Service";
    return {
      id: jobId,
      ref: `JC-${String(index + 1).padStart(6, "0")}`,
      date: dateOffset(index % 10),
      customerId: customer.id,
      mileage: 18000 + index * 1350,
      recordMode: "OFFICIAL",
      isArchived: false,
      demoSample: true,
      notes: index % 4 === 0 ? "Demo customer requested quick service." : "",
      items: [{
        id: itemId,
        categoryId: service.category.id,
        itemId: service.item.id,
        attendantId: employee.id,
        action: index % 3 === 0 ? "Checked" : index % 3 === 1 ? "Changed" : "Repaired",
        status: index % 3 === 0 ? "Checked" : index % 3 === 1 ? "Changed" : "Repaired",
        confirmed: index % 2 === 0,
        amount,
        subItemIds: (service.item.subItems || []).slice(0, 2).map((item) => item.id),
        subItemStatus: {},
        remarks: index % 2 === 0 ? "Completed in demo workflow." : "",
        categoryName: service.category.name,
        serviceItemName: service.item.name,
        attendantName: employee.name
      }],
      purchaseItems: index % 5 === 0 ? [{
      ...
      serviceCard: isGeneral ? {
        serviceMileage: 18000 + index * 1350,
        nextServiceMileage: 23000 + index * 1350,
        nextServiceDate: addMonths(dateOffset(index % 10), 3),
        lines: [{
          id: `demo-service-card-${index + 1}`,
          jobItemId: itemId,
          itemName: service.item.name,
          checked: true,
          done: true,
          remarks: "Sample service card line."
        }]
      } : undefined
    };
  });

  demo.payments = demo.jobCards.slice(0, 16).map((job, index) => {
    const jobItem = job.items[0];
    const amount = index % 4 === 0 ? Math.round(jobItem.amount * 0.6) : jobItem.amount;
    return {
      id: `demo-pay-${String(index + 1).padStart(2, "0")}`,
      ref: `RCPT-JC${String(index + 1).padStart(6, "0")}`,
      date: job.date,
      jobId: job.id,
      method: index % 3 === 0 ? "Mobile Money" : index % 3 === 1 ? "Cash" : "Bank",
      ...
      items: [{
        ...
        attendantId: jobItem.attendantId,
        attendantName: jobItem.attendantName,
        amount
      }]
    };
  });
  demo.expenses = [
    ["Utilities", 18000], ["Consumables", 22000], ["Transport", 15000], ["Maintenance", 35000], ["Rent", 120000]
    ...
    amount,
    comment: "Demo expense",
    recordMode: "OFFICIAL",
    isArchived: false,
    demoSample: true
  }));
  demo.commissions = demo.payments.slice(0, 12).map((payment, index) => {
    const job = demo.jobCards.find((entry) => entry.id === payment.jobId);
    const item = job?.items?.[0] || {};
    return {
      id: `demo-comm-${index + 1}`,
      date: payment.date,
      jobId: payment.jobId,
      attendantId: item.attendantId || demo.employees[0].id,
      type: "Amount",
      rate: "",
      amount: Math.round((payment.items?.[0]?.amount || 0) * 0.1),
      recordMode: "OFFICIAL",
      isArchived: false,
      demoSample: true
    };
...
async function init() {
  ...
  const loadedFromMysql = DEMO_MODE ? false : await hydrateStateFromBackend({ render: false });
  renderAll();
  if (!DEMO_MODE && !loadedFromMysql) scheduleRemoteSave(100);
  enforceSecurityLock();
...
function loadState() {
  if (DEMO_MODE) return loadDemoState();
  try {
...
function loadDemoState() {
  ...
  const lastResetDay = localStorage.getItem(DEMO_RESET_DAY_KEY);
  const sampleVersion = localStorage.getItem(DEMO_SAMPLE_VERSION_KEY);
  if (lastResetDay !== todayKey || sampleVersion !== DEMO_SAMPLE_VERSION) {
    const demoState = buildDemoSampleState();
    localStorage.setItem(STORE_KEY, JSON.stringify(demoState));
    localStorage.setItem(DEMO_RESET_DAY_KEY, todayKey);
    localStorage.setItem(DEMO_SAMPLE_VERSION_KEY, DEMO_SAMPLE_VERSION);
    localStorage.setItem("vasma_sync_status", "demo");
    return demoState;
  }
  try {
    const saved = JSON.parse(localStorage.getItem(STORE_KEY));
    return saved ? mergeState(buildDemoSampleState(), saved) : buildDemoSampleState();
  } catch {
    return buildDemoSampleState();
  }
}
...
function resetDemoDataIfNeeded() {
  if (!DEMO_MODE) return;
  const todayKey = today();
  if (localStorage.getItem(DEMO_RESET_DAY_KEY) === todayKey && localStorage.getItem(DEMO_SAMPLE_VERSION_KEY) === DEMO_SAMPLE_VERSION) return;
  state = buildDemoSampleState();
  save({ localOnly: true });
  localStorage.setItem(DEMO_RESET_DAY_KEY, todayKey);
  localStorage.setItem(DEMO_SAMPLE_VERSION_KEY, DEMO_SAMPLE_VERSION);
  localStorage.setItem("vasma_sync_status", "demo");
...
function scheduleDemoMidnightReset() {
  if (!DEMO_MODE) return;
  const now = new Date();
  const midnight = new Date(now);
  midnight.setHours(24, 0, 0, 0);
  window.setTimeout(() => {
    localStorage.removeItem(DEMO_RESET_DAY_KEY);
    state = buildDemoSampleState();
    save({ localOnly: true });
    localStorage.setItem(DEMO_SAMPLE_VERSION_KEY, DEMO_SAMPLE_VERSION);
    renderAll();
    toast("Demo panel reset to sample data for the new day.");
    scheduleDemoMidnightReset();
...
function viewJobCards() {
  return recordsForView(state.jobCards).filter((job) => tableRows(job.items).some((item) => isServiceEnabled(serviceKeyForCategory(byId(state.categories, item.categoryId)))));
}
...
function rememberDemoRegistration(session) {
  if (!DEMO_MODE || !session) return;
  const registeredAt = session.demoRegisteredAt || session.trialStartDate || new Date().toISOString();
  const demoId = localStorage.getItem("vasma_demo_registration_id") || `demo-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  localStorage.setItem("vasma_demo_registration_id", demoId);
  fetch(`${API_URL}/api/public/demo-registration`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      id: demoId,
      packageCode: session.serviceCode,
      packageName: session.packageName || session.serviceCode,
      allowedServices: session.allowedServices || [],
      ...
      registeredAt,
      trialDays: session.trialDays || TRIAL_DAYS
    })
  }).catch(() => {});
}
...
function promptBusinessProfileSetup() {
  ...
  openModal(`
    ...
    <form id="businessProfileForm" class="form-grid">
      ...
      <div class="wide feature-list">
        ${(session.allowedServices || ["tire", "carWash", "general"]).map((service) => `<span class="pill">${escapeHtml(serviceLabel(service))}</span>`).join("")}
      </div>
  ...
  document.querySelector("#businessProfileForm").addEventListener("submit", (event) => {
    ...
    const requestedServices = session.allowedServices?.length ? [...session.allowedServices] : ["tire", "carWash", "general"];
    ...
    rememberDemoRegistration(updated);
...
function installDate() {
  const session = businessSession();
  if (session?.demoRegisteredAt) return String(session.demoRegisteredAt).slice(0, 10);
  if (session?.trialStartDate) return session.trialStartDate;
...
function renderSettings() {
  ...
  document.querySelector("#demoChangePackage")?.addEventListener("click", resetDemoPackageSelection);
...
function resetDemoPackageSelection() {
  if (!confirm("Reset the demo PIN and return to package selection? User-added demo data will be cleared.")) return;
  [
    "vasma_token",
    "vasma_user",
    "vasma_login_time",
    "vasma_business_session",
    STORE_KEY,
    LICENSE_STORE_KEY,
    INSTALL_DATE_KEY,
    DEMO_RESET_DAY_KEY,
    DEMO_SAMPLE_VERSION_KEY
  ...
  const sections = {
    business: `
      <div class="settings-section">
        ...
        <div class="actions">
          <button type="button" class="secondary" id="demoChangePackage">Reset PIN and Choose Another Package</button>
        </div>
      </div>`,
    theme: `
      <div class="settings-section">
