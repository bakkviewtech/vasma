...
async function pushStateToBackend() {
  if (hydratingFromBackend || !getAuthToken()) return;
  try {
    const payload = { ...state, _sync: { savedAt: new Date().toISOString(), appVersion: APP_VERSION } };
    await apiRequest("/api/data/save", { method: "POST", body: { data: payload } });
  ...
  if (!isServiceEnabled("general")) {
    views.serviceCards.innerHTML = `
      <div class="panel stack">
        <h2>Service Cards</h2>
        <p class="muted">General Service is not authorized for this business setup. Enter an admin authorization code in Settings > Service Setup to enable it.</p>
      </div>`;
    return;
  }
...
function renderSettings() {
  ...
  document.querySelector("#settingsForm").addEventListener("submit", (event) => {
    ...
    if ((localStorage.getItem("vasma-settings-section") || "business") === "services") {
      const requested = [...event.target.querySelectorAll("[name=enabledServices]:checked")].map((input) => input.value);
      const authCode = (event.target.querySelector("[name=serviceAuthCode]")?.value || "").trim().toUpperCase();
      if (!requested.length) return toast("Select at least one service.");
      if (!verifyServiceAuthorization(requested, authCode)) return toast("Invalid admin authorization code for selected services.");
      data.enabledServices = requested;
      data.serviceAuthorization = {
        code: authCode,
...
function settingsSectionMarkup(section, settings) {
  const sections = {
    ...
    services: serviceSetupMarkup(settings),
    theme: `
      <div class="settings-section">
        <h2>Theme</h2>
        <div class="form-grid">
...
function serviceSetupMarkup(settings) {
  const enabled = Array.isArray(settings.enabledServices) ? settings.enabledServices : ["tire", "carWash", "general"];
  const auth = settings.serviceAuthorization || {};
  return `
    <div class="settings-section stack">
      <h2>Service Setup</h2>
      <p class="muted">Choose the services authorized for this business. Changes require an admin authorization code from the developer/admin agreement.</p>
      <div class="grid three">
        ${SERVICE_OPTIONS.map((service) => `
          <label class="check-card">
            <input type="checkbox" name="enabledServices" value="${service.key}" ${enabled.includes(service.key) ? "checked" : ""}>
            <span><strong>${service.label}</strong><small>${service.category}</small></span>
          </label>`).join("")}
      </div>
      <div class="form-grid">
        <label class="wide">Admin Authorization Code<input name="serviceAuthCode" placeholder="Example: VASMA-ALL-SERVICES" autocomplete="off"></label>
      </div>
      <table>
        <tbody>
          <tr><th>Current Authorized Services</th><td>${enabled.map(serviceLabel).join(", ")}</td></tr>
          <tr><th>Last Authorization Code</th><td>${escapeHtml(auth.code || "Not recorded")}</td></tr>
          <tr><th>Last Authorized</th><td>${auth.authorizedAt ? new Date(auth.authorizedAt).toLocaleString() : "Not recorded"}</td></tr>
        </tbody>
      </table>
      <p class="muted">Available admin codes: VASMA-TIRE, VASMA-CARWASH, VASMA-GENERAL, VASMA-TIRE-CARWASH, VASMA-TIRE-GENERAL, VASMA-CARWASH-GENERAL, VASMA-ALL-SERVICES.</p>
    </div>`;
}
...
function serviceCodeKey(services) {
  return [...services].sort().join("|");
}

function verifyServiceAuthorization(services, code) {
  return SERVICE_AUTH_CODES[serviceCodeKey(services)] === String(code || "").trim().toUpperCase();
}
